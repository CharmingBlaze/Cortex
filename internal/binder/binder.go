...
type Binder struct {
	...
	libName   string
	// includeHeader is emitted in generated #include (e.g. "SDL2/SDL.h"); empty => "<libName>.h"
...
// parseDefinesContent extracts object-like #define constants from (possibly preprocessed) source text.
func (b *Binder) parseDefinesContent(content string) {
...
// SetIncludeHeader sets the #include used in generated bindings (e.g. "SDL2/SDL.h").
func (b *Binder) SetIncludeHeader(h string) {
	b.includeHeader = strings.TrimSpace(h)
}

func (b *Binder) generatedIncludeLine() string {
	if b.includeHeader != "" {
		if strings.HasPrefix(b.includeHeader, "<") && strings.HasSuffix(b.includeHeader, ">") {
			return "#include " + b.includeHeader + "\n"
		}
		if strings.HasPrefix(b.includeHeader, "\"") && strings.HasSuffix(b.includeHeader, "\"") {
			return "#include " + b.includeHeader + "\n"
		}
...
func (b *Binder) GenerateCortex() string {
	...
	sb.WriteString(b.generatedIncludeLine())
	sb.WriteString("\n")
	...
	for _, enum := range b.enums {
		...
		for _, v := range enum.Values {
			if v.Value != "" {
				...
				sb.WriteString(fmt.Sprintf("// const int %s = <implicit>;\n", v.Name))
			}
		}
		sb.WriteString("\n")
	}
...
func (b *Binder) generateFunctionBinding(fn CFunction) string {
	...
	if fn.ReturnType.IsPointer && fn.ReturnType.Name != "char" && !fn.ReturnType.IsFunctionPointer {
		sb.WriteString("// Returns pointer; lifetime is library-defined — see docs/POINTER_FREE_AND_FFI.md\n")
	}
	if fn.ReturnType.IsFunctionPointer {
		sb.WriteString("// TODO: returns function pointer; verify calling convention / manual @c binding\n")
	}
	for _, p := range fn.Params {
		if p.Type.IsFunctionPointer {
			sb.WriteString("// TODO: function pointer parameter(s); verify with @c or a hand-written prototype\n")
			break
		}
	}
