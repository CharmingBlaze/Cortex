static void cortex_lambda_0(gui_event e) {
                    {
                        gui_dialog_confirm(0, "Confirm", "Do you want to proceed?", on_confirm_result);
}}
...
void on_button_click(gui_event e) {    {
        gui_dialog_info(0, "Hello", "Button was clicked!");
}}
void on_checkbox_change(gui_event e) {    {
        int checked = gui_event_get_bool(e);
if (checked) {
            gui_dialog_info(0, "Checkbox", "Checkbox is now checked!");
}
void on_confirm_result(gui_event e) {            {
                int confirmed = gui_event_get_bool(e);
if (confirmed) {
...
gui_widget checkbox = gui_checkbox_create("Enable Feature", on_checkbox_change);
gui_widget entry = gui_entry_create("Type here...", NULL);
gui_widget confirm_btn = gui_button_create("Show Confirm", cortex_lambda_0);
...
gui_container_add(vbox, confirm_btn);
